...
JSON
    ...
    # ── Patch openclaw.json directly (no plugins install --link) ─────────────
    ...
    #
    # Instead we manage openclaw.json directly:
    #   1. Ensure plugin_dir is in plugins.load.paths (the actual load mechanism)
    #   2. Remove openclaw-goal-agent from plugins.allow if present
    #      (load.paths is sufficient; allow causes installs to trigger a second load)
...
# 2. Remove from allow list — allow + installs causes a second load (36 conflicts)
